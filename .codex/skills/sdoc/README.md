...
- 自动输出：执行摘要、关键结构表、主流程步骤、Mermaid 流程图、难点说明、示例代码片段
- 关键结构、代码片段、模块关系均输出 `/repo/path.rs#L123` 风格源码链接，方便从 Markdown 直接跳到实现
- Rust 代码会识别 `impl` 内方法归属，职责说明优先结合字段、状态读写、局部调用链、协议不变量和领域关键词推断
- 支持 CLI + Python 库接口
...
5. **职责语义推断**：对 Rust `impl` 方法归属、结构体字段、精确函数名、调用关系和常见协议不变量生成更专业的职责说明。
6. **源码链接**：所有关键位置输出项目根目录相对链接，例如 `/codex/codex-rs/core/src/thread_manager.rs#L120`。
7. **难点说明库**：对 token 预算、调用/输出配对、多模态降级、上下文差量、异步、缓存、抽象层、状态流转等模式生成阅读提示。
8. **并发解析**：多文件目录使用线程池并发解析。
9. **缓存复用**：按文件内容哈希缓存分析结果，避免重复解析未变更文件。
10. **可扩展语言解析器**：通过 `LANGUAGE_SPECS` 注册新语言。
11. **可配置输出规模**：可控制扫描深度、最大文件数、片段数量等，避免文档失控膨胀。
